...
class CreateStringInspector : BaseLocalInspectionTool() {
    override fun checkFile(file: PsiFile, manager: InspectionManager, isOnTheFly: Boolean): Array<ProblemDescriptor>? {
        ...
        val errors = errorList.filter { it.code == "undefined_getter" && it.message.contains("defined for the class 'S'.") }
        if (errors.isEmpty()) return null

        return errors.map {
            val string = PsiTreeUtil.findElementOfClassAtOffset(file, it.offset, DartReferenceExpression::class.java, false)!!
