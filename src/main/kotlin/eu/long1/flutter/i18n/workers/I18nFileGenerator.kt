...
class I18nFileGenerator(private val project: Project) {
    ...
    init {
        try { psiManager = PsiManager.getInstance(project) } catch (ignored: Throwable) { }
        try { documentManager = PsiDocumentManager.getInstance(project) } catch (ignored: Throwable) { }
    }
    ...
    private fun stringFiles(valuesFolder: VirtualFile): ArrayList<VirtualFile>? {
        return valuesFolder.children?.filter {
    ...
    private fun createFileForLang(
        lang: String,
        psiManager: PsiManager,
        documentManager: PsiDocumentManager,
        valuesFolder: VirtualFile
    ): VirtualFile? {
        val virtualFile = valuesFolder.findOrCreateChildData(this, "strings_$lang.arb")
        return (psiManager.findFile(virtualFile) as? JsonFile)?.let { psiFile ->
